...
class MssqlSchemaParser extends BaseSchemaParser
{
    ...
    {
        $stmt = $this->dbh->query("SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_TYPE = 'BASE TABLE' AND TABLE_NAME <> 'dtproperties' AND TABLE_NAME <> 'sysdiagrams'");

        // First load the tables (important that this happen before filling out details of tables)
        $tables = array();
        // Get array of excluded table names
        $excludes = array_map('trim', explode(',', $this->getGeneratorConfig()->getBuildProperty('migrationExcludes')));
        while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
            $name = $this->cleanDelimitedIdentifiers($row[0]);
            // Exclude migration table and tables in exclusion list
            if (($name == $this->getMigrationTable()) || (in_array($name, $excludes))) {
                continue;
            }
    ...
    protected function addForeignKeys(Table $table)
    {
        ...
        $stmt = $this->dbh->query("SELECT ccu1.CONSTRAINT_NAME, ccu1.TABLE_NAME, ccu1.COLUMN_NAME, ccu2.TABLE_NAME AS FK_TABLE_NAME, ccu2.COLUMN_NAME AS FK_COLUMN_NAME
